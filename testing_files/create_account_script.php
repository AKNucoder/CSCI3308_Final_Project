...
<body>
	...
	if (!empty($username) and !empty($password) and !empty($confirm_password) and !empty($first_name) and !empty($last_name)
		...
		if ($username_validated && $password_validated && $email_validated && $name_validated && $phone_validated && $city_validated && $state_validated && $zip_validated){
			...
			$My_SQL_new_user_creation = $My_SQL_create_new_user_profile . $My_SQL_create_new_user_contact;
			//echo $My_SQL_create_new_user_profile;
			//echo $My_SQL_create_new_user_contact;

			echo $My_SQL_new_user_creation . "<br>";
			$query_sent = mysql_query($My_SQL_new_user_creation);
			
			if (!$query_sent){
				echo "Error: Could not save data to database. Contact support. <br>";
			}
			else{
	...
	function validate_username($username){
		...
		if ($return_result!=False){
     	...
     	/*
     	if ($return_username != NULL && $return_username != $username){
     		$is_validated = True;
     	}
     	*/
     	return $is_validated;
